...
//import api
import { getSingleFaq, getFaqCategory, UpdateFaq } from '../../api/faqCategory'
...
const Update = () => {
  ...
  const Submit = async (e) => {
    try {
      ...
      const { success, error, message } = await UpdateFaq(reqData)
      if (success) {
        toast(
          {
            message: message,
  ...
  const fetchSingleFaq = async () => {
    try {
      ...
      const { success, result } = await getSingleFaq(decryptData)
      if (success) {
        let data = {
