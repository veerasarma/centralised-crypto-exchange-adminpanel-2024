...
//import action
import { orderHistory, orderHistoryDoc } from "../../api/inverse/history";
//import lib
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ state, row: { original } }) => {
          return (
            <Fragment>
              <CFormLabel>{momentFormat(original.closedAt)}</CFormLabel>
            </Fragment>
          );
        },
      },
      {
        ...
        accessor: "pnl",
        Cell: ({ state, row: { original } }) => {
          return original && parseFloat(original.pnl).toFixed(8);
        },
      },
    ],
    []
