...
//import action
import { getPair } from "../../api/inverse/pair";
//import lib
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ state, row: { original } }) => {
          let status =
            original && original.botstatus === "off" ? "Off" : "Active";
        ...
        Cell: ({ state, row: { original } }) => {
          ...
          return (
            <CButton
              className="edit-btn"
              size="sm"
              onClick={() => history("/inverse-edit-pair/" + encryptData)}
            >
              {" "}
  ...
  return (
    <CCard className="mb-4">
      <CCardHeader>
        <CButton
          className="add-btn"
          onClick={() => history("/inverse-add-pair")}
        >
