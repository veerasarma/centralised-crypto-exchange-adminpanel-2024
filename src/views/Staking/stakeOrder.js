...
import { orderXLS, orderCSV, satkeorderPDF } from './downloader'
...
//import api
import { StakeOrderList } from 'src/api/Staking/staking'
import {
  DateColumnFilter,
  SelectColumnFilter,
  StatusColumnFilter,
...
import { stakeOrderTypeFilter } from 'src/components/Table/ServerSide/filters'
...
const List = () => {
  const columns = useMemo(
    () => [
      ...
      {
        Header: 'End Date',
        accessor: 'settleEndDate',
        Filter: DateColumnFilter,
        Cell: ({ state, row: { original } }) => {
          console.log(original, 'originaloriginaloriginal')
          return (
            <Fragment>
              <CFormLabel>{momentFormat(original.settleEndDate)}</CFormLabel>
            </Fragment>
          )
        },
      },
  ...
  const fetchFaqData = async ({ pageIndex, pageSize, filters, sortBy, type }) => {
    try {
      ...
      const { success, result, errors } = await StakeOrderList(reqData)
      console.log('successresulterrors', success, result, errors)
      if (success==true) {
        setData(result && result.result ? result.result.data:[])
        setCount(result && result.result ?result.result.count:0)
  ...
  const handleDoc = async (type) => {
    try {
      ...
      if (Object.keys(dateObj).length > 0 && dateObj.startDate != '' && dateObj.endDate != '') {
        ...
        let { result, data } = await StakeOrderList(docType)
        if (type && type === 'pdf') {
          satkeorderPDF(result.result.pdfData)
