...
//import api
import { SubModulesList, DeleteSubModule } from 'src/api/submodules'
...
const List = () => {
          ...
          return (
            <>
              <CButton
                className="edit-btn"
                size="sm"
                onClick={() => history('/update-submodules/' + encryptData)}
              >
  ...
  const fetchFaqData = async ({ pageIndex, pageSize, filters, sortBy }) => {
    try {
      setLoading(true)
      console.log(pageIndex, pageSize, 'iii')
      ...
      const { success, result } = await SubModulesList(reqData)
  ...
  return (
    <>
      <CCard className="mb-4">
        <CCardHeader>
          <CButton className="add-btn" onClick={() => history('/add-submodules')}>
            <CIcon icon={cilPlus}></CIcon>Add submodule
          </CButton>
        </CCardHeader>
        <CCardBody>
          <ServerSideTable
