  ...
  try {
    ...
    data.map((item) => {
      resultArr.push([
        ...
        item.execPrice,
        item.flag == true ? 'Market' : item.price,
        item.filledQuantity,
        item.orderType == 'market' && item.buyorsell == 'buy'
          ? item.filledQuantity
          : item.openQuantity,
        item.averagePrice,
        item.status == 'cancel' ? "Cancelled" : capitalize(item.status),
      ])
    })
    const unit = 'pt'
