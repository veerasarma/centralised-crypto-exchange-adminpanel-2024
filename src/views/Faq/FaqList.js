...
//import api
import { getFaqList, DeleteFaq } from '../../api/faqCategory'
...
const FAQList = () => {
          ...
          return (
            <>
              <CButton
                className="edit-btn"
                size="sm"
                onClick={() => history('/updatefaq/' + encryptData)}
              >
  ...
  const fetchFaqData = async ({ pageIndex, pageSize, filters, sortBy }) => {
    try {
      ...
      const { success, result } = await getFaqList(reqData)
  ...
  const deleteRecord = async (record, state) => {
    try {
      ...
      const { success, error, message } = await DeleteFaq(data)
      if (success) {
        toast(
          {
            message: message,
  ...
  return (
    <>
      <CCard className="mb-4">
        <CCardHeader>
          <CButton className="add-btn" onClick={() => history('/addfaq')}>
            <CIcon icon={cilPlus}></CIcon>Add Faq
          </CButton>
        </CCardHeader>
        <CCardBody className="faq_list_table">
          <ServerSideTable
            columns={columns}
            data={data}
