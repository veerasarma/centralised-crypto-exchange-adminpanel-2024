...
//import api
import { getSubModules, getSingleRole, UpdateRole } from 'src/api/role'
...
const Update = () => {
  ...
  const Submit = async (e) => {
    try {
      ...
      const { success, error, message } = await UpdateRole(reqData)
      if (success) {
        toast(
          {
            message: message,
  ...
  const fetchSingleData = async () => {
    try {
      ...
      const { success, result } = await getSingleRole(decryptData)
      if (success) {
        let data = {
          role: result.role,
  ...
  return (
    <CRow>
      <CCol xs={6} sm={6} md={6}>
        <CCard>
          <CCardBody>
            <CForm className="bg-opaque">
              <div className="mb-3">
                <CFormLabel htmlFor="exampleFormControlInput1">Restriction</CFormLabel>
                <Select
                  styles={colourStyles}
                  value={
                    option.option && option.option.length > 0
                      ? option.option.filter((el) => {
                        if (restriction && restriction.includes(el.value)) {
                          return el
                        }
                      })
                      : []
              ...
              {/* <div className="mb-3">
                ...
                <CFormSelect id="role" value={role} onChange={handleChange}>
