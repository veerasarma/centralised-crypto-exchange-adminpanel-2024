...
export const GlobalFilter = (props) => {
  const { preGlobalFilteredRows, globalFilter, setGlobalFilter, state, fetchData } = props
  ...
  return (
    <span>
      ...
      <DatePicker
        selected={
          new Date().getTimezoneOffset() < 0
            ? new Date(startDate).getTime() - Math.abs(new Date().getTimezoneOffset()) * 60 * 1000
            : new Date().getTimezoneOffset() > 0
            ? new Date(startDate).getTime() + Math.abs(new Date().getTimezoneOffset()) * 60 * 1000
        ...
        onChange={(date) => {
          date =
            new Date().getTimezoneOffset() < 0
              ? date.getTime() + Math.abs(new Date().getTimezoneOffset()) * 60 * 1000
              : new Date().getTimezoneOffset() > 0
              ? date.getTime() - Math.abs(new Date().getTimezoneOffset()) * 60 * 1000
              : date.getTime()
          date = new Date(date)
      ...
      <DatePicker
        selected={
          new Date().getTimezoneOffset() < 0
            ? new Date(endDate).getTime() - Math.abs(new Date().getTimezoneOffset()) * 60 * 1000
            : new Date().getTimezoneOffset() > 0
            ? new Date(endDate).getTime() + Math.abs(new Date().getTimezoneOffset()) * 60 * 1000
  ...
  return (
    <>
      <DatePicker
        selected={
          new Date().getTimezoneOffset() < 0
            ? new Date(filterValue).getTime() - Math.abs(new Date().getTimezoneOffset()) * 60 * 1000
            : new Date().getTimezoneOffset() > 0
            ? new Date(filterValue).getTime() + Math.abs(new Date().getTimezoneOffset()) * 60 * 1000
...
export function StatusColumnFilter({ column: { filterValue, setFilter, preFilteredRows, id } }) {
  ...
  const options = React.useMemo(() => {
    ...
    if (id == 'status') {
      ...
      let StatusArray = ['pending', 'approved', 'new']
      StatusArray.forEach((row) => {
        options.add(row)
      })
    } else if (id == 'level') {
      let StatusArray = ['Basic', 'Medium', 'Advanced']
      StatusArray.forEach((row) => {
        options.add(row)
      })
    }
...
export function UserSatusFillter({ column: { filterValue, setFilter, preFilteredRows, id } }) {
  const options = React.useMemo(() => {
    const options = new Set()
...
export function PairStatusFillter({ column: { filterValue, setFilter, preFilteredRows, id } }) {
  const options = React.useMemo(() => {
    const options = new Set()
    console.log('...id', id)
    if (id == 'botstatus') {
