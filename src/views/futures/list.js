...
//import action
import { getPair } from '../../api/futures/pair'
//import lib
...
const Pairs = () => {
    ...
    () => [
      {
        ...
        accessor: 'baseCoinSymbol',
        // Filter: DefaultColumnFilter,
      },
      {
        Header: 'Quote Currency',
        accessor: 'quoteCoinSymbol',
        // Filter: DefaultColumnFilter,
      },
      {
        Header: 'Bot Status',
        ...
        Cell: ({ state, row: { original } }) => {
          let status = original && original.botstatus === 'off' ? 'Off' : 'Active'
        ...
        Cell: ({ state, row: { original } }) => {
          ...
          return (
            <CButton
              className="edit-btn"
              size="sm"
              onClick={() => history('/Future-edit-pair/' + encryptData)}
            >
              {' '}
  ...
  return (
    <CCard className="mb-4">
      <CCardHeader>
        <CButton className="add-btn" onClick={() => history('/Future-add-pair')}>
