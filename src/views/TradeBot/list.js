...
//import action
import { getBotList, cancelBot } from '../../api/spot/pair'
//import lib
...
const Pairs = () => {
                    ...
                    return (
                        <>
                            <CButton
                                className="edit-btn"
                                size="sm"
                                onClick={() => history('/edit-bot/' + encryptData)}
                            >
                                {' '}
                            ...
                            <CButton
                                ...
                                size="sm"
                                disabled={false}
                                onClick={() => { forceCancel(original._id) }}
                            >
                                Force Cancel
                            </CButton>
                        </>
    ...
    const fetchPairs = async ({ pageIndex, pageSize, filters, sortBy }) => {
        ...
        let { success, result } = await getBotList(reqData)
        setLoading(false)
        if (success) {
    ...
    return (
        <CCard className="mb-4">
            <CCardHeader>
                <CButton className="add-btn" onClick={() => history('/add-bot')}>
                    <CIcon icon={cilPlus}></CIcon>Add Bot
                </CButton>
                <CButton className="add-btn" onClick={() => history('/bot-user')}>
